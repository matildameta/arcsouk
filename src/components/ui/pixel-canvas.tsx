...
// Plain class — safe to define on the server (extends nothing).
class Pixel {
  width: number;
  height: number;
...
// The custom element extends HTMLElement, which only exists in the browser.
// Define + register it lazily on the client so SSR never evaluates it.
let _registered = false;
function registerPixelCanvas() {
  if (typeof window === 'undefined' || typeof HTMLElement === 'undefined') return;
  if (_registered || customElements.get('pixel-canvas')) {
    _registered = true;
    return;
  }
